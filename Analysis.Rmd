...
a. I computed the correlation matrix with all 11 varaiables, and sorted the matrix.
...
The correlation coefficient r measures the strength and direction of a linear relationship between two variables on a scatterplot and can be intrepreted as follows:

          1 A perfect uphill (positive) linear relationship      
        .70 A strong linear relationship
        .50 A moderate linear relationship
        .30 A weak linear relationship
...
qplot(am, mpg, data=mtcars, geom=c("boxplot", "jitter"), fill=factor(am),  main="Mileage by Transmission Type", xlab="", ylab="Miles per Gallon") 
```